...
__device__ 
void compute_next_state(gpe_state_t *in, gpe_state_t *out, gpe_param_t *params, double dt){
...
__device__ 
void compute_currents(gpe_state_t *in, gpe_state_t *out, gpe_param_t *params){
    double v = in -> voltage;
    ...
    out->I_Ca = -(params->g_Ca * pow(gpe_sinf(v), 2) * (v - params->E_Ca));
    out->I_ahp = -(params->g_ahp * (v - params->E_ahp) * (in->CA/(in->CA + 10)));
}


__device__ 
void compute_gating(gpe_state_t *in, gpe_state_t *out, gpe_param_t *params, double dt){
    double v = in->voltage;
    out->H = in->H + dt * 0.05 * ((gpe_hinf(v) - in->H)/gpe_tauh(v));
    out->R = in->R + dt * 1.0 * (gpe_rinf(v) - in->R)/30.0;
